...
class Institutional_invoice extends CORE_Controller
{
    ...
    function __construct() {
        ...
        $this->load->model('Institutional_invoice_model');
        $this->load->model('Institutional_invoice_items_model');
    ...
    public function index() {
        ...
        $data['title'] = 'Institutional Invoice';
        
        (in_array('15-2',$this->session->user_rights)? 
        $this->load->view('institutional_invoice_view', $data)
    ...
    function transaction($txn = null,$id_filter=null) {
        switch ($txn){
            case 'list':
            $m_pf_invoice = $this->Institutional_invoice_model;
            $response['data']=$this->response_rows(array('institutional_invoice.is_active'=>TRUE, 'institutional_invoice.is_deleted'=>FALSE));
            ...
            case 'items': //items on the specific PO, loads when edit button is called
                $m_items=$this->Institutional_invoice_items_model;
                $response['data']=$m_items->get_list(
                    array('institutional_invoice_id'=>$id_filter),
                    array(
                        'institutional_invoice_items.*',
                        'products.product_code',
                        'products.product_desc',
                        'units.unit_id',
                        'units.unit_name'
                    ...
                    array(
                        array('products','products.product_id=institutional_invoice_items.product_id','left'),
                        array('units','units.unit_id=institutional_invoice_items.unit_id','left')
                    ),
                    'institutional_invoice_items.institutional_item_id ASC'
                );
            ...
            case 'create':
                $m_invoice=$this->Institutional_invoice_model;
                ...
                $institutional_invoice_id=$m_invoice->last_insert_id();

                $m_invoice_items=$this->Institutional_invoice_items_model;
                ...
                for($i=0;$i<count($prod_id);$i++){
                    ...
                    $m_invoice_items->institutional_invoice_id=$institutional_invoice_id;
                ...
                $m_invoice->institutional_inv_no='INS-INV-'.date('Ymd').'-'.$institutional_invoice_id;
                $m_invoice->modify($institutional_invoice_id);
                ...
                if($m_invoice->status()===TRUE){
                    ...
                    $response['msg'] = 'Institutional Invoice successfully created.';
                    $response['row_added']=$this->response_rows($institutional_invoice_id);
            ...
            case 'update':
                $m_invoice=$this->Institutional_invoice_model;
                $institutional_invoice_id=$this->input->post('institutional_invoice_id',TRUE);
                ...
                $m_invoice_items=$this->Institutional_invoice_items_model;
                $m_invoice_items->delete_via_fk($institutional_invoice_id);
                ...
                for($i=0;$i<count($prod_id);$i++){
                    ...
                    if($m_invoice->status()===TRUE){
                        $response['title'] = 'Success!';
                        $response['stat'] = 'success';
                        $response['msg'] = 'Institutional Invoice successfully updated.';
                        $response['row_updated']=$this->response_rows($institutional_invoice_id);
    ...
    function response_rows($filter_value){
        return $this->Institutional_invoice_model->get_list(
            $filter_value,
            array(
                'institutional_invoice.*',
                'DATE_FORMAT(institutional_invoice.date_invoice,"%m/%d/%Y") as date_invoice',
                'DATE_FORMAT(institutional_invoice.date_due,"%m/%d/%Y") as date_due',
                // 'departments.department_id',
                // 'departments.department_name',
                // 'customers.customer_name',
                // 'institutional_invoice.salesperson_id',
                // 'institutional_invoice.address',
                // 'sales_order.so_no'
            ),
            array(
                // array('departments','departments.department_id=institutional_invoice.department_id','left'),
                // array('customers','customers.customer_id=institutional_invoice.customer_id','left'),
                // array('sales_order','sales_order.sales_order_id=institutional_invoice.sales_order_id','left'),
            ),
            'institutional_invoice.institutional_invoice_id DESC'
        );
    }
