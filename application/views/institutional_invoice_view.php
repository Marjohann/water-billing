...
<ol class="breadcrumb"  style="margin-bottom: 0;">
    ...
    <li><a href="Institutional_invoice">Institutional  Invoice</a></li>
</ol>
<div class="container-fluid"">
<div data-widget-group="group1">
<div class="row">
<div class="col-md-12">
<div id="div_institutional_invoice_list">
    <div class="panel panel-default">
        <div class="panel-body table-responsive">
        <div class="row panel-row">
        <h2 class="h2-panel-heading">Institutional Invoice</h2><hr>
            <table id="tbl_institutional_invoice" class="table table-striped" cellspacing="0" width="100%" style="">
                <thead >
                <tr>
                    <th></th>
                    <th>Invoice #</th>
...
<div id="div_institutional_invoice_fields" style="display: none;">
    <div class="panel panel-default">
        <div class="pull-right">
            <h4 class="institutional_invoice_title" style="margin-top: 0%;"></h4>
        ...
        <div class="row panel-row">
            <form id="frm_institutional_invoice" role="form" class="form-horizontal">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><br />
                ...
                <div class="row" style="display: none;">
                    <div class="col-lg-4 col-lg-offset-8">
                        <div class="table-responsive">
                            <table id="tbl_institutional_invoice_summary" class="table invoice-total" style="font-family: tahoma;">
                                <tbody>
                                <tr>
                                    <td>Discount :</td>
...
$(document).ready(function(){
    ...
    var initializeControls=function(){
        dt=$('#tbl_institutional_invoice').DataTable({
            "dom": '<"toolbar">frtip',
            "bLengthChange":false,
            "ajax" : "Institutional_invoice/transaction/list",
            "language": {
            ...
            "columns": [
                ...
                { targets:[1],data: "institutional_inv_no" },
        ...
        var createToolBarButton=function(){
            var _btnNew='<button class="btn btn-success" id="btn_new" style="text-transform: none;font-family: Tahoma, Georgia, Serif; " data-toggle="modal" data-placement="left" title="Record Institutional Invoice" >'+
                '<i class="fa fa-plus"></i> Record Institutional Invoice</button>';
    ...
    var bindEventHandlers=(function(){
        var detailRows = [];
        $('#tbl_institutional_invoice tbody').on( 'click', 'tr td.details-control', function () {
            var tr = $(this).closest('tr');
            var row = dt.row(tr);
            var d=row.data();

            window.open('Templates/layout/institutional-invoice/'+ d.institutional_invoice_id+'?type=contentview');
        } );
        ...
        $('#btn_new').click(function(){
            _txnMode="new";
            clearFields($('#div_institutional_invoice_fields'));
        ...
        $('#tbl_institutional_invoice tbody').on('click','button[name="edit_info"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.institutional_invoice_id;
            ...
            _txnMode="edit";
            $('.institutional_invoice_title').html('Edit Sales Invoice');
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.institutional_invoice_id;
            ...
            $.ajax({
                url : 'Institutional_invoice/transaction/items/'+data.institutional_invoice_id,
                type : "GET",
                cache : false,
                dataType : 'json',
                processData : false,
            ...
            $('#span_invoice_no').html(data.institutional_inv_no);
            $('#txt_overall_discount').val(accounting.formatNumber($('#txt_overall_discount').val(),2)); 
            showList(false);
        }
        });
        $('#tbl_institutional_invoice tbody').on('click','button[name="remove_info"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.institutional_invoice_id;
            _count=data.count;
        ...
        $('#btn_save').click(function(){
            if(validateRequiredFields($('#frm_institutional_invoice'))){
                if(_txnMode=="new"){
                    CreateInstitutionalInvoice().done(function(response){
                        showNotification(response);
                        dt.row.add(response.row_added[0]).draw();
                        clearFields($('#frm_institutional_invoice'));
                        showList(true);
                    }).always(function(){
                        showSpinningProgress($('#btn_save'));
                    });
                }else{
                    updateInstitutionalInvoice().done(function(response){
                        showNotification(response);
    ...
    var CreateInstitutionalInvoice=function(){
        var _data=$('#frm_institutional_invoice,#frm_items').serializeArray();
        var tbl_summary=$('#tbl_institutional_invoice_summary');
        ...
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Institutional_invoice/transaction/create",
            "data":_data,
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
    var updateInstitutionalInvoice=function(){
        var _data=$('#frm_institutional_invoice,#frm_items').serializeArray();
        var tbl_summary=$('#tbl_institutional_invoice_summary');
        _data.push({name : "remarks", value : $('textarea[name="remarks"]').val()});
        ...
        _data.push({name : "summary_tax_amount", value : tbl_summary.find(oTableDetails.inv_tax_amount).text()});
        _data.push({name : "summary_after_tax", value : tbl_summary.find(oTableDetails.after_tax).text()});
        _data.push({name : "institutional_invoice_id" ,value : _selectedID});
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Institutional_invoice/transaction/update",
            "data":_data,
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
    var removeIssuanceRecord=function(){
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Institutional_invoice/transaction/delete",
            "data":{institutional_invoice_id : _selectedID}
        });
    };
    var showList=function(b){
        if(b){
            $('#div_institutional_invoice_list').show();
            $('#div_institutional_invoice_fields').hide();
            $('.datepicker.dropdown-menu').hide();
        }else{
            $('#div_institutional_invoice_list').hide();
            $('#div_institutional_invoice_fields').show();
        }
    };
