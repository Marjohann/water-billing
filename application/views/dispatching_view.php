...
<ol class="breadcrumb"  style="margin-bottom: 0;">
    ...
    <li><a href="Dispatching">Warehouse Dispatching</a></li>
</ol>
...
<div id="div_dispatching_invoice_fields" style="display: none;">
    <div class="panel panel-default">
        <div class="panel-body" >
        <div class="row panel-row">
            <form id="frm_dispatching_invoice" role="form" class="form-horizontal">
                <h2 class="h2-panel-heading">Invoice # : <span id="span_invoice_no">DIS-YYYY-MM-DD-XXXX</span></h4>
                <div>
                <hr>
                    <div class="row">
                        <div class="col-sm-4">
                        ...
                        <div class="col-sm-3">
                            ...
                            <div class="input-group">
                                <input type="text" name="inv_no" class="form-control" required data-error-msg="Invoice Reference is required." readonly>
                                <input type="hidden" name="is_sales" class="form-control">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"><br />
                ...
                <div class="row" style="display: none;">
                    <div class="col-lg-4 col-lg-offset-8">
                        <div class="table-responsive">
                            <table id="tbl_warehouse_dispatching_summary" class="table invoice-total" style="font-family: tahoma;">
                                <tbody>
                                <tr>
                                    <td>Discount :</td>
...
<div id="modal_so_list" class="modal fade" tabindex="-1" role="dialog"><!--modal-->
    <div class="modal-dialog" style="width: 80%;">
        <div class="modal-content">
            <div class="modal-header ">
                <button type="button" class="close"   data-dismiss="modal" aria-hidden="true">X</button>
                <h2 class="modal-title" style="color: white;"><span id="modal_mode"> </span>Sales Invoices | Cash Invoices</h2>
            </div>
            <div class="modal-body">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="container-fluid" style="overflow: scroll; width: 100%;">
                    <DispatchingInvoice id="sales_invoice">
                    </DispatchingInvoice>
                </div>
            </div>
        </div>
...
$(document).ready(function(){
    ...
    var initializeControls=function(){
        dt=$('#tbl_warehouse_dispatching').DataTable({
            ...
            "columns": [
                ...
                { targets:[1],data: "dispatching_inv_no" },
        ...
        dt_so=$('#tbl_inv_list').DataTable({
            ...
            "columns": [
                ...
                { targets:[5],data: "order_status" },
                {
                    targets:[6],data: null,
                    render: function (data, type, full, meta){
                        var btn_accept_si='<button class="btn btn-success btn-sm" name="accept_si"  style="margin-left:-15px;text-transform: none;" data-toggle="tooltip" data-placement="top" title=""><i class="fa fa-check"></i> Accept SI</button>';
                        var btn_accept_ci='<button class="btn btn-success btn-sm" name="accept_ci"  style="margin-left:-15px;text-transform: none;" data-toggle="tooltip" data-placement="top" title=""><i class="fa fa-check"></i> Accept CI</button>';

                        if(data.order_status=="Closed"){
                            return '';
                        }else{
                             if(data.is_sales == 1){ return '<center>'+btn_accept_si+'</center>'; }else{ return '<center>'+btn_accept_ci+'</center>'; }
                        }
    ...
    var bindEventHandlers=(function(){
        var detailRows = [];
        $('#tbl_warehouse_dispatching tbody').on( 'click', 'tr td.details-control', function () {
            var tr = $(this).closest('tr');
            var row = dt.row(tr);
            var d=row.data();
            window.open('Templates/layout/dispatching-invoice/'+ d.dispatching_invoice_id+'?type=contentview');
        } );
        ...
        $('#tbl_inv_list tbody').on( 'click', 'tr td.details-control', function () {
            var tr = $(this).closest('tr');
        ...
        $('#tbl_inv_list > tbody').on('click','button[name="accept_ci"]',function(){
            _selectRowObjCi=$(this).closest('tr');
            var data=dt_so.row(_selectRowObjCi).data();
            ...
            resetSummary();
            $.ajax({
                url : 'Dispatching/transaction/item-balance-ci/'+data.invoice_id,
                type : "GET",
                cache : false,
                dataType : 'json',
                processData : false,
        ...
        $('#tbl_inv_list > tbody').on('click','button[name="accept_si"]',function(){
            _selectRowObjSi=$(this).closest('tr');
            var data=dt_so.row(_selectRowObjSi).data();
            $('input,textarea').each(function(){
                var _elem=$(this);
                $.each(data,function(name,value){
                    if(_elem.attr('name')==name&&_elem.attr('type')!='password'){
            ...
            resetSummary();
            $.ajax({
                url : 'Dispatching/transaction/item-balance-si/'+data.invoice_id,
                type : "GET",
                cache : false,
                dataType : 'json',
                processData : false,
        ...
        $('#tbl_warehouse_dispatching tbody').on('click','button[name="edit_info"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.dispatching_invoice_id;
            _count=data.count;
            _is_journal_posted=data.is_journal_posted;
            if(data.sales_invoice_id != 0){
                $("input[name=is_sales]").val('1'); 
                $("input[name=inv_no]").val(data.sales_inv_no); 
            }else if(data.cash_invoice_id !=0){
                $("input[name=is_sales]").val('2'); 
                $("input[name=inv_no]").val(data.cash_inv_no);
            }
            ...
            $.ajax({
                url : 'Dispatching/transaction/items/'+data.dispatching_invoice_id,
                type : "GET",
                cache : false,
                dataType : 'json',
                processData : false,
            ...
            $('#span_invoice_no').html(data.dispatching_inv_no);
            showList(false);
        }
        });
        $('#tbl_warehouse_dispatching tbody').on('click','button[name="remove_info"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.dispatching_invoice_id;
            _count=data.count;
        ...
        $('#btn_yes').click(function(){
            ...
            //}else{
            removeDispatching().done(function(response){
                showNotification(response);
        ...
        $('#btn_save').click(function(){
            if(validateRequiredFields($('#frm_dispatching_invoice'))){
                if(_txnMode=="new"){
                    createDispatchingInvoice().done(function(response){
                        showNotification(response);
                        dt.row.add(response.row_added[0]).draw();
                        clearFields($('#frm_dispatching_invoice'));
                        showList(true);
                    }).always(function(){
                        showSpinningProgress($('#btn_save'));
                    });
                }else{
                    updateDispatchingInvoice().done(function(response){
                        showNotification(response);
    ...
    var createDispatchingInvoice=function(){
        var _data=$('#frm_dispatching_invoice,#frm_items').serializeArray();
        var tbl_summary=$('#tbl_warehouse_dispatching_summary');
        ...
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Dispatching/transaction/create",
            "data":_data,
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
    var updateDispatchingInvoice=function(){
        var _data=$('#frm_dispatching_invoice,#frm_items').serializeArray();
        var tbl_summary=$('#tbl_warehouse_dispatching_summary');
        _data.push({name : "remarks", value : $('textarea[name="remarks"]').val()});
        ...
        _data.push({name : "summary_tax_amount", value : tbl_summary.find(oTableDetails.inv_tax_amount).text()});
        _data.push({name : "summary_after_tax", value : tbl_summary.find(oTableDetails.after_tax).text()});
        _data.push({name : "dispatching_invoice_id" ,value : _selectedID});
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Dispatching/transaction/update",
            "data":_data,
            "beforeSend": showSpinningProgress($('#btn_save'))
        });
    };
    var removeDispatching=function(){
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Dispatching/transaction/delete",
            "data":{dispatching_invoice_id : _selectedID}
        });
    };
    var showList=function(b){
        if(b){
            $('#dis_dispatching_invoice_list').show();
            $('#div_dispatching_invoice_fields').hide();
            $('.datepicker.dropdown-menu').hide();
        }else{
            $('#dis_dispatching_invoice_list').hide();
            $('#div_dispatching_invoice_fields').show();
        }
    };
