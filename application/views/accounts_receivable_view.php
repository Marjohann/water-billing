    ...
    <div class="panel-group panel-default" id="accordionA">
        ...
        <div class="panel panel-default">
            ...
            <div id="" class="">
                <div class="panel-body">
            <a data-toggle="collapse" data-parent="#accordionA" href="#collapseTwo" style="text-decoration: none;">
            <h2 class="h2-panel-heading">Review Billing Penalties Receivable (Pending)</h2><hr>
            </a>
                    <div >
                    <table id="tbl_billing_penalties_receivables" class="table table-striped" cellspacing="0" width="100%">
                        <thead class="">
                        <tr>
                            <th>&nbsp;</th>
                            <th>Month</th>
...
$(document).ready(function(){
    ...
    var _cboCustomerType; var dtBillingPenaltyReview;
    ...
    var initializeControls=function(){
        ...
        dtBillingPenaltyReview=$('#tbl_billing_penalties_receivables').DataTable({
            "bLengthChange":false,
            "ajax" : "Penalty_sending/transaction/penalties-for-review",
            "columns": [
                {
                    "targets": [0],
                    "class":          "details-control",
                ...
                { targets:[2],data: "year" },
                { targets:[3],data: "total_penalty_amount", render: $.fn.dataTable.render.number( ',', '.', 2), sClass: "right-align" },
    ...
    var bindEventHandlers=function(){
        ...
        $('#tbl_billing_penalties_receivables tbody').on( 'click', 'tr td.details-control', function () {
            var tr = $(this).closest('tr');
            var row = dtBillingPenaltyReview.row( tr );
            var idx = $.inArray( tr.attr('id'), detailRows );
            ...
            if ( row.child.isShown() ) {
                ...
                $.ajax({
                    "dataType":"html",
                    "type":"POST",
                    "url":"Penalty_sending/transaction/billing-receivable-penalty-for-review?id="+ d.billing_penalty_id,
                    "beforeSend" : function(){
                        row.child( '<center><br /><img src="assets/img/loader/ajax-loader-lg.gif" /><br /><br /></center>' ).show();
                    }
                }).done(function(response){
                    ...
                    var tbl=$('#tbl_entries_for_review_'+ d.billing_penalty_id);
                    var parent_tab_pane=$('#journal_review_'+ d.billing_penalty_id);

                    reInitializeDropDownAccounts(tbl);
                    reInitializeChildEntriesTable(tbl);
                    reInitializeChildElementsBillingPenalty(parent_tab_pane);
    ...
    var reInitializeChildElementsBillingPenalty=function(parent){
        var _dataParentID=parent.data('parent-id');
        var btn=parent.find('button[name="btn_finalize_journal_review"]');
        var btnClose=parent.find('button[name="btn_close_journal_review"]');
            ...
            if(isBalance('#tbl_entries_for_review_'+_dataParentID)){
                finalizeJournalReview().done(function(response){
                    ...
                    if(response.stat=="success"){
                        dt.row.add(response.row_added[0]).draw();
                        var _parentRow=_curBtn.parents('table.table_journal_entries_review').parents('tr').prev();
                        dtBillingPenaltyReview.row(_parentRow).remove().draw();
                    }
                }).always(function(){
                    showSpinningProgress(_curBtn);
                });
