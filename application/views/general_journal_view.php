...
<!--                 <div class="panel-heading">
        ...
        <div class="panel panel-default">
                <div class="panel-body table-responsive">
                    <h2 class="h2-panel-heading">Review Connection Deposits</h2><hr>
                    <div class="row-panel">
                        <table id="tbl_deposit_review" class="table table-striped" cellspacing="0" width="100%">
                            <thead class="">
                            <tr>
                                <th></th>
                                <th>Batch #</th>
            ...
            <div class="panel-body table-responsive" >
              ...
              <!--    <div class="panel-ctrls" data-actions-container="" data-action-collapse='{"target": ".panel-body, .panel-footer"}'></div> -->
                            ...
                            <button id="btn_browse_recurring" class="btn btn-primary" style="margin-bottom: 10px; text-transform: capitalize;"><i class="fa fa-folder-open-o"></i> Browse Recurring Template</button>
                                ...
                                <div id="div_check">
                                    <input type="checkbox" name="chk_save" id="checkcheck">&nbsp;&nbsp;<label for="checkcheck"><strong>Save as Template</strong></label>
                                </div>
                                <div id="div_no_check">
                                    <br>
                                    <br>
...
$(document).ready(function(){
    ...
    var dtReviewAdjustment; var dtReviewDeposits;
    ...
    var initializeControls=function(){
        ...
        dtReviewDeposits=$('#tbl_deposit_review').DataTable({
            "bLengthChange":false,
                // "order": [[ 1, "desc" ]],
            "ajax" : "General_journal/transaction/list-deposit-for-review",
            "columns": [
                {
                    "targets": [0],
                    "class":          "details-control",
                ...
                { sClass:"right_align", targets:[4],data: "batch_total_deposit" , render: $.fn.dataTable.render.number( ',', '.', 2)}
            ]
        });
    ...
    var bindEventHandlers=function(){
        ...
        $('#tbl_accounts_receivable tbody').on( 'click', 'tr td.details-control', function () {
            ...
            $('#tbl_deposit_review tbody').on( 'click', 'tr td.details-control', function () {
                var tr = $(this).closest('tr');
                var row = dtReviewDeposits.row( tr );
                var idx = $.inArray( tr.attr('id'), detailRows );
                ...
                if ( row.child.isShown() ) {
                    ...
                    $.ajax({
                        "dataType":"html",
                        "type":"POST",
                        "url":"Connection_sending/transaction/batch-deposit-for-review?id="+ d.service_connection_batch_id,
                        "beforeSend" : function(){
                            row.child( '<center><br /><img src="assets/img/loader/ajax-loader-lg.gif" /><br /><br /></center>' ).show();
                        }
                    }).done(function(response){
                        ...
                        var tbl=$('#tbl_entries_for_review_dep_'+ d.service_connection_batch_id);
                        var parent_tab_pane=$('#journal_review_'+ d.service_connection_batch_id);
                        reInitializeDropDownAccounts(tbl);
                        reInitializeChildEntriesTable(tbl);
                        reInitializeChildElementsDeposit(parent_tab_pane);
    ...
    //              user defines
        ...
        var reInitializeChildElementsDeposit=function(parent){
            ...
            parent.on('click','button[name="btn_finalize_journal_review"]',function(){
                var _curBtn=$(this);
                if(isBalance('#tbl_entries_for_review_dep_'+_dataParentID)){
                    if(validateRequiredFields('#tbl_entries_for_review_dep_'+_dataParentID)){
                    finalizeJournalReview().done(function(response){
                        ...
                        if(response.stat=="success"){
                            ...
                            dtReviewDeposits.row(_parentRow).remove().draw();
                        }
                    }).always(function(){
        ...
        var reInitializeChildElements=function(parent){
            ...
            parent.on('click','button[name="btn_close_journal_review"]',function(){
                ...
                if(parent.find('input[name="closing_reason"]').val()=='' || parent.find('input[name="closing_reason"]').val() == null){
                    ...
                    CloseInvoiceIssuance().done(function(response){
                        showNotification(response);
                        if(response.stat=="success"){
                            var _parentRow=_curBtnClose.parents('table.table_journal_entries_review').parents('tr').prev();
                            dtReview.row(_parentRow).remove().draw();
            ...
            var CloseInvoiceIssuance=function(){
                var _dataClose=[]
                _dataClose.push({ name:'issuance_department_id', value: _dataParentID});
                _dataClose.push({ name:'closing_reason', value: parent.find('input[name="closing_reason"]').val()});
                _dataClose.push({ name:'trn_type', value: parent.find('input[name="trn_type"]').val()});
                return $.ajax({
                    "dataType":"json",
                    "type":"POST",
                    "url":"Issuance_department/transaction/close-invoice",
                    "data":_dataClose,
                    "beforeSend": showSpinningProgress(btnClose)
                });
            };
